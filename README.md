...
5. FFmpeg ve Deno kurulumda gelmez: ilk açılışta Format Seçimi sayfasındaki **FFmpeg Güncelle** ve **YouTube / Deno** düğmelerini kullanın (WinGet ile kurulur). yt-dlp EXE'ye gömülü değildir; `pylibs` klasöründen yüklenir ve **yt-dlp Güncelle** ile yeni setup gerekmeden güncellenir (sonrasında yeniden başlatın). `pylibs` herhangi bir nedenle eksikse uygulama başlangıçta PyPI'den kendisi indirir (öz onarım). TLS/curl-cffi EXE ile birlikte gelir.
...
24. yt-dlp EXE'ye dondurulmamalı; setup sürümünde `{app}\pylibs` klasöründen yüklenmeli ve **yt-dlp Güncelle** düğmesi PyPI wheel'lerini indirip pylibs'i yerinde güncellemeli (pip kullanılmaz, yeniden başlatma istenir). pylibs eksikse dondurulmuş uygulama başlangıçta öz onarım ile PyPI'den kendisi indirmeli; indirme başarısızsa açıklayıcı hata gösterilmeli. Kaynak sürümde pip tabanlı güncelleme korunmalı; `IS_FROZEN` ve dondurulmuş EXE'de kurulum klasörünü tarayan FFmpeg/FFprobe keşfi aynen kalmalı.
